...
def custom_train_detector(model,
    ...
    # modify by yufc 0921 add load ckpt
    if cfg.resume_from:
        # 手动加载checkpoint
        checkpoint = torch.load(cfg.resume_from)
        # 获取模型的状态字典
        model_state_dict = model.state_dict()
        # 过滤并加载已有模型的权重，忽略新模块部分
        ...
        # 将状态字典加载回模型
        logger.info(f"Yufc: Loaded model checkpoint (excluding optimizer state). from {cfg.resume_from}")
    ...
    # #     # runner.optimizer.load_state_dict({}) # 这里要清理优化器状态，因为 resume 覆盖了正确的优化器
    # elif cfg.load_from:
