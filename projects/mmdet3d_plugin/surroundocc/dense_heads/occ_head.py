...
# from projects.mmdet3d_plugin.surroundocc.detectors.adapter import OccupancyAdapter
# from projects.mmdet3d_plugin.surroundocc.detectors.feadapter import FEAdapter
from projects.mmdet3d_plugin.surroundocc.detectors.occAdapter import OccAdapter
...
class OccHead(nn.Module): 
    def __init__(self,
        ...
        self.adapter = OccAdapter() # adapter OccupancyAdapter FEAdapter
    ...
    def forward(self, mlvl_feats, mlvl_feats_prev, img_metas, IFs):
        ...
        for i in range(self.fpn_level):
            ...
            view_features_detach = view_features.detach()#freeze
            
            volume_embed_i = self.transformer[i](
                [view_features_detach],
                volume_queries,
                volume_h=volume_h,
                volume_w=volume_w,
                volume_z=volume_z,
            ...
            volume_embed_i_detach = volume_embed_i.detach()#freeze参数
            volume_embed.append(volume_embed_i_detach)
        ...
        for i in range(len(self.deblocks)):
            ...
            result_detach = result.detach()#freeze参数

            if i in self.out_indices:
                outputs.append(result_detach)
            elif i < len(self.deblocks) - 2:  # we do not add skip c,onnection at level 0
                volume_embed_temp = volume_embed_reshape.pop()
                result_detach = result_detach + volume_embed_temp
            
        # adapter
        occ_displacement = self.adapter(mlvl_feats, mlvl_feats_prev, IFs)
        # final output
        occ_preds = []
        for i in range(len(outputs)):
            occ_pred = self.occ[i](outputs[i])
            occ_pred_detach = occ_pred.detach()#freeze参数
            
            # occ_pred = occ_pred + occ_displacement[i]
            occ_preds.append(occ_pred_detach)
        occ_preds[3] = occ_preds[3] + occ_displacement
        # occ_preds
        # [1, 7, 25, 25, 2]
        # [1, 17, 50, 50, 4]
        # [1, 17, 100, 100, 8]
        ...
        outs = {
            ...
            'occ_preds': occ_preds,
            # 'occ_displacement': occ_displacement
        }
        return outs
    ...
    def loss(self,
        ...
        if not self.use_semantic:
            ...
            for i in range(len(preds_dicts['occ_preds'])):
                ...
                # displacement = preds_dicts['occ_displacement'][i][:, 0]
                # 可以给dispalcement加点正则：含义是不让displacement过大
