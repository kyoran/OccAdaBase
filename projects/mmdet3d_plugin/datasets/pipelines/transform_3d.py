...
class PadMultiViewImage(object):
    ...
    def _pad_img(self, results):
        ...
        results['pad_fixed_size'] = self.size  # 这两个字段不进行特殊处理了
        results['pad_size_divisor'] = self.size_divisor  # 这两个字段不进行特殊处理了
        ...
        for i in range(0, len(results['img_prev'])):
            if self.size is not None:
                padded_img = [mmcv.impad(
                    img, shape=self.size, pad_val=self.pad_val) for img in results['img_prev'][i]]
            elif self.size_divisor is not None:
                padded_img = [mmcv.impad_to_multiple(
                    img, self.size_divisor, pad_val=self.pad_val) for img in results['img_prev'][i]]
            ori_shape_prev_list.append([img.shape for img in results['img_prev'][i]])
            img_prev_list.append(padded_img)
            img_shape_prev_list.append([img.shape for img in padded_img])
            pad_shape_prev_list.append([img.shape for img in padded_img])
        results['ori_shape_prev'] = ori_shape_prev_list
        results['img_prev'] = img_prev_list
        results['img_shape_prev'] = img_shape_prev_list
        results['pad_shape_prev'] = pad_shape_prev_list
        return
...
class NormalizeMultiviewImage(object):
    ...
    def __call__(self, results):
        ...
        # prev
        for i in range(0, len(results['img_prev'])):
            results['img_prev'][i] = [mmcv.imnormalize(img, self.mean, self.std, self.to_rgb) for img in results['img_prev'][i]]
            results['img_norm_cfg_prev'][i] = dict(
                mean=self.mean, std=self.std, to_rgb=self.to_rgb)
        assert(len(results['img_norm_cfg_prev']) == len(results['img_prev']))
        return results
...
class PhotoMetricDistortionMultiViewImage:
    ...
    def __call__(self, results):
        ...
        """
        imgs = results['img']
        imgs_prev_lst = results['img_prev']
        ...
        results['img'] = process_T(imgs)
        imgs_prev_output_lst = []
        for i in range(0, len(results['img_prev'])):
            imgs_prev_output_lst.append(process_T(results['img_prev'][i]))
        results['img_prev'] = imgs_prev_output_lst
        return results
...
class CustomCollect3D(object):
    ...
    def __call__(self, results):
        ...
        for key in self.meta_keys:
            ...
            # prev
            if key + "_prev" in results:
                img_metas[key + "_prev"] = results[key + "_prev"]

        data['img_metas'] = DC(img_metas, cpu_only=True)
        for key in self.keys:
            data[key] = results[key]
        # 特殊处理一下 img_prev 字段，img 字段应该是在DefaultFormatBundle3D里面被DataContainer包起来的
        # 里面没有处理 img_prev 字段，这里特殊处理一下
        img_attributes = {
            'stack': data['img'].stack,
            'padding_value': data['img'].padding_value,
            'cpu_only': data['img'].cpu_only,
            'pad_dims': data['img'].pad_dims,
        }
        # data['img_prev'] = DC(results['img_prev'], **img_attributes)
    ...
    def _calc_frame_difference(self, rgb_files):
        ...
        # current frame
        # print("last_frame:", last_frame.shape) # 不要最后一张图了
