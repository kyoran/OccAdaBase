...
@PIPELINES.register_module()
class MyDefaultFormatBundle(object):
    """Default formatting bundle.
    ...
    def __call__(self, results):
        ...
        if 'img' in results:
            ...
            for i in range(0, len(results['img_prev'])):
                if isinstance(results['img_prev'][i], list):
                    # process multiple imgs in single frame
                    imgs = [img.transpose(2, 0, 1) for img in results['img_prev'][i]]
                    imgs = np.ascontiguousarray(np.stack(imgs, axis=0))
                    results['img_prev'][i] = DC(to_tensor(imgs), stack=True)
                else:
                    img = np.ascontiguousarray(results['img_prev'][i].transpose(2, 0, 1))
                    results['img_prev'][i] = DC(to_tensor(img), stack=True)
...
# for debug
@PIPELINES.register_module()
class MyDefaultFormatBundle3D(MyDefaultFormatBundle):
    """Default formatting bundle.
    ...
    def __init__(self, class_names, with_gt=True, with_label=True):
        super(MyDefaultFormatBundle3D, self).__init__()
                ...
                if 'gt_names_3d' in results:
                    results['gt_labels_3d'] = np.array([
                        ...
                        for n in results['gt_names_3d']
                                                       ...
                                                       dtype=np.int64)
        results = super(MyDefaultFormatBundle3D, self).__call__(results)
        return results
